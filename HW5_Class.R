...
## ------------------------------------------------------------------
## Validity: return TRUE or a character vector of errors
setValidity("sparse_numeric", function(object) {
  errs <- character()
  
  ## slot types & basic shapes
  ...
  ## positions: 1..length, sorted, unique
  ...
  ## values: no explicit zeros stored
...
## ensure same overall length
.check_same_len <- function(x, y) {
  if (x@length != y@length)
    stop("Operands must have the same 'length'.")
}

## merge add/sub: two-pointer sweep over sorted 'pos'
...
## elementwise product on overlap
...
## construct a valid sparse_numeric from (pos,val,len)
...
## ------------------------------------------------------------------
## plot(x, y): plot overlapping non-zero indices
setMethod("plot",
          signature(x = "sparse_numeric", y = "sparse_numeric"),
          function(x, y, ...) {
            .check_same_len(x, y)
...
## ------------------------------------------------------------------
## Extra method of choice: length() returns full logical length
